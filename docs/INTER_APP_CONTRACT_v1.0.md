# DEVIS SIGN-OFF WORKFLOW — INTER-APP CONTRACT v1.0-rc3

**Status:** Draft, pending Step 3 confirmation by Archidoc (Archisign confirmed v1.0-rc2 with explicit pre-commitment to higher rc tags; rc3 changes are Archidoc-internal §2 only).
...
| Archidoc | R3 §6 + §7 + §7.3 + §7.4 | pending — rc2 review returned `confirmed-with-correction` per §A.1; rc3 confirmation expected by return per Archidoc commitment | — | — |
| Archisign | R3 §7.1–§7.7 + §8 | `confirmed v1.0-rc2` with explicit pre-commitment to higher rc tags (rc3 changes are Archidoc-internal §2 only, do not touch §3 or §4 Archisign-owned content) | Archisign | 2026-04-25 |

**Contract freezes at v1.0 when Archidoc replies `confirmed v1.0-rc3`.** (Archisign already confirmed v1.0-rc2 with explicit extension to higher rc tags; rc3 changes are confined to §2 Archidoc-internal table routing and two cosmetic fixes, none of which touch Archisign-owned sections.)
