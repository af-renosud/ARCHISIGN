...
| ARCHISIGN_WEBHOOK_V2_TENANTS     | env    | No (legacy) | Legacy allowlist override. Accepts CSV (`architrak,otherTenant`) **or** JSON map (`{"architrak":"<key>"}`). When set, ONLY listed tenants get v2 dual-emit; when unset, default-on applies. Malformed values WARN at startup and disable v2 for everyone. |
| ARCHISIGN_WEBHOOK_V2_DISABLED_TENANTS | env | No   | CSV of tenant keys to opt out of v2 dual-emit. Only consulted when `ARCHISIGN_WEBHOOK_V2_TENANTS` is unset (default-on mode). |
...
## Recent Changes
- 2026-05-08: **Task #20 — v2 tenant allowlist parser hardened**: `isV2Enabled` no longer relies on a naive `.split(",")`. New `parseV2TenantConfig` in `server/services/WebhookSignature.ts` format-detects CSV vs JSON-map (first non-whitespace `{` or `[` triggers JSON parse; map keys become the allowlist, values ignored — true per-tenant key isolation remains a separate workstream). Malformed values resolve to an empty allowlist + `parseError` instead of silently disabling v2. New `validateV2TenantConfig()` runs once at boot from `server/index.ts` (before `httpServer.listen`) and emits a single `[startup] WARN:` line on parse failure or `[startup] INFO:` line on success/intentionally-empty/default-on. The parsed result is cached so per-event `isV2Enabled` lookups don't re-parse. Default flipped to **opt-out**: when `ARCHISIGN_WEBHOOK_V2_TENANTS` is unset, every tenant gets v2 unless listed in the new `ARCHISIGN_WEBHOOK_V2_DISABLED_TENANTS` CSV. Legacy allowlist takes precedence (if set, it is the only source of truth, even alongside the disabled list). Unit tests in `server/services/__tests__/WebhookSignature.test.ts` cover CSV happy path, JSON-map happy path, malformed JSON, JSON array, empty/whitespace, single/multi-tenant, default-on with/without opt-outs, and precedence rules — runnable via `npx tsx --test server/services/__tests__/WebhookSignature.test.ts`. Closes the silent-v2-disable failure mode behind envelope 22 (Architrak's `{"architrak":"<key>"}` value was treated as a one-element CSV literal `{"architrak":"<key>"}` and the verifier 401'd).
